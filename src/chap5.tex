...
\chapter{Modelli statistici e stima}
    ...
    \end{obsv}
        \begin{defn}[Distorsione]\label{defn:Distorsione}
            ...
            \begin{equation}\label{eq:Distorsione}
                ...
                \text{E}_{\vartheta}\left[d_n(X_1,\, \ldots,\, X_{n})\right] - k(\vartheta)
            .\end{equation}
            Una notazione alternativa è la seguente: \[
    ...
    \section{Stimatore non distorto}
        \begin{defn}
            ...
            \emph{corretto} per $k(\vartheta)$ se vale: \[
                \forall \vartheta \,:\, \text{bias}_{\vartheta}(d_n) = 0
            .\]
        \end{defn}
        \begin{prty}\label{prty:MLE_varianza_bias}
            Se $D_n = d_n(X_1,\, \ldots,\, X_{n})$ è uno stimatore di $k(\vartheta)$ basato sul campione 
             $X_1,\, \ldots,\, X_{n}$ che ammette momento secondo finito, allora possiamo scrivere l'errore quadratico 
             medio come: \[
                \scriptstyle MSE \textstyle_{\vartheta}(D_n) = 
                \text{Var}_{\vartheta}(D_n) + \left(\text{bias}_{\vartheta}(D_n)\right)^2
             ;\] allora se $D_n$ è uno stimatore non distorto per $k(\vartheta)$ otteniamo: \[
                \text{bias}_{\vartheta}(D_n) = 0 \implies \scriptstyle MSE \textstyle_{\vartheta}(D_n) = 
                \text{Var}_{\vartheta}[d_n(\vec{X})]
             ,\] cioè l'errore quadratico medio coincide con la varianza dello stimatore.
        \end{prty}
        \begin{proof}
            Partendo dalla scrittura dell'errore quadratico medio (\ref{eq:Errore_quadratico_medio}) abbiamo:
            \begin{align*}
                \scriptstyle \text{MSE} \textstyle_{\vartheta}(D_n) &= 
                \text{E}_{\vartheta} \left[(D_n - \text{E}_{\vartheta}[D_n] + \text{E}_{\vartheta}[D_n] - d_{\vartheta})^2\right]\\
                &= \underset{\bigstar}{\underbrace{\text{E}_{\vartheta} \left[(D_n - \text{E}_{\vartheta}[D_n])^2\right]}} +
                \underset{\blacktriangle}{\underbrace{\left(\text{E}_{\vartheta} [D_n] - d_n\right)^2}} + 
                2 \cdot \underset{\blacksquare}{\underbrace{\cancel{\text{E}_{\vartheta} [D_n - \text{E}_{\vartheta} [D_n]]}}} \cdot (\text{E}_{\vartheta}[D_n] - d_n)\\
                &= \underset{\bigstar}{\underbrace{\text{Var}_{\vartheta}(D_n)}} + \underset{\blacktriangle}{\underbrace{\text{bias}_{\vartheta}(D_n)^2}}
            .\end{align*}
            Nella dimostrazione abbiamo usato rispettivamente per $\bigstar$ la Definizione~\ref{defn:Varianza}, 
            per $\blacktriangle$ la Definizione~\ref{defn:Distorsione} e infine per $\blacksquare$ abbiamo 
            un termine che si annulla, infatti dalle Proprietà~\ref{prty:Valore_atteso} (per questo esempio mettiamo da parte la scrittura in funzione del parametro $\vartheta$):
            \[
                \mathbb{E}[D - \mathbb{E}[D]] = \mathbb{E}[D] - \mathbb{E}[\mathbb{E}[D]] = 
                \mathbb{E}[D] - \mathbb{E}[D] = 0
            .\]
        \end{proof}
    \section{Proprietà asintotiche degli stimatori}
        ...
        \subsection{Consistenza debole}
            \begin{defn}
                Una successione di stimatori $(D_n)_n = (d_n(X_1,\, \ldots,\, X_{n}))_n$ è detta \emph{debolmente consistente} per $k(\vartheta)$ se vale: \[
                    ...
                    P_{\vartheta}\left\{|D_n - k(\vartheta)| \geq \varepsilon\right\} \rightarrow 0
                .\] 
            \end{defn}
        \subsection{Consistenza forte}
            \begin{defn}
                Una successione di stimatori $(D_n)_n = (d_n(X_1,\, \ldots,\, X_{n}))_n$ è detta \emph{fortemente consistente} per $k(\vartheta)$ se vale: \[
                    \forall \vartheta \in \Theta \,:\, P_{\vartheta}\left\{\lim_{n \to \infty} D_n =k(\vartheta)\right\} = 1
                .\] 
            \end{defn}
        \subsection{Consistenza in media quadratica}
            ...
            \begin{prty}
                ...
                \begin{enumerate}
                    \item $\text{bias}_{\vartheta}(D_n) \rightarrow 0 \land \text{Var}_{\vartheta}[D_n] \rightarrow 0 
                        \iff \scriptstyle \text{MSE} \textstyle_{\vartheta}(D_n) \rightarrow 0$;
            ...
            \begin{proof}
                \hfill
                \begin{enumerate}
                    \item Segue dalla Proprietà~\ref{prty:MLE_varianza_bias};
                    \item Sfruttando l'Equazione~\ref{eq:Disuguaglianza_Markov} affermiamo
                        che: \[
                        P\left\{|D_n - d_{\vartheta}| \geq \varepsilon\right\} \leq \frac{1}{\varepsilon^2} \mathbb{E}[|D_n - d_{\varepsilon}|^2]
                    ;\]
                    la disuguaglianza più esterna si verifica dato che, per la consistenza in 
                    media quadratica tende a $0$ il termine $\mathbb{E}[|D_n - d_{\varepsilon}|^2] \rightarrow 0$.
                \end{enumerate}
            \end{proof}
        \subsection{Normalità asintotica}
            \begin{defn}
                ...
                la distribuzione asintotica degli stimatori è normale (secondo il Teorema~\ref{thm:Teorema_limite_centrale}): \[
                    \lim_{n \rightarrow +\infty} P_{\vartheta}\left\{\sqrt{n} \frac{D_n - k(\vartheta)}{h(\vartheta)} \leq x\right\} = \Phi(x)
                .\] Se esiste una funzione $h(\vartheta)$ per cui l'equazione precedente è verificata, allora la successione degli stimatori ha \emph{varianza asintotica} $h(\vartheta)^2$.
            \end{defn}
    ...
    \end{note}
        \begin{defn}
            ...
            .\end{align}
        \end{defn}
        \begin{prty}\label{prty:media_campionaria}
        ...
        \end{proof}
        \begin{note}
            Dalle Proprietà~\ref{prty:media_campionaria} e dal Teorema~\ref{thm:Legge_grandi_numeri_forte} possiamo affermare che la \emph{media empirica}, come la media campionaria, sia uno stimatore non distorto e inoltre fortemente consistente della media.
        \end{note}
    \section{Proprietà della varianza campionaria}
        \begin{defn}\label{defn:Varianza_campionaria}
            ...
            .\]
        \end{defn}
        \begin{prty}\label{prty:varianza_campionaria}
        ...
        \end{proof}
        \begin{note}
            Dalle Proprietà~\ref{prty:varianza_campionaria} e dal Teorema~\ref{thm:Legge_grandi_numeri_forte} possiamo 
            affermare che la \emph{varianza empirica}, al contrario della varianza campionaria, sia uno stimatore 
            distorto (per $n$ fissato). La varianza campionaria usa il fattore $\frac{1}{n-1}$ per correggere la distorsione.
        \end{note}
    \section{Massima verosimiglianza}
        ...
        \begin{obsv}[Log-verosimiglianza]
            ...
            \emph{log\nbdash verosimiglianza}: \[
                \ln \big(\mathcal{L}(\vartheta,\, \vec{x})\big) \overset{\ast}{=} \sum_{i=1}^{n} \ln(F_{\vartheta}(x_i))
            .\] L'uguaglianza $\ast$ sfrutta il fatto che il logaritmo di un prodotto (MLE), come da 
            Definizione~\ref{defn:MLE}, sia uguale alla somma dei logaritmi dei termini.
        \end{obsv}
    ...
    \section{Metodo dei momenti}
        \begin{defn}
            ...
            \begin{equation*}
                ...
                \end{cases}
            \end{equation*}
            Otteniamo un sistema di $k$ equazioni nelle incognite $\vartheta_1,\, \ldots,\, \vartheta_k$.
    ...
    \section{Intervalli di confidenza}
        ...
        \begin{defn}[Intervallo illimitato superiormente]
            Consideriamo una statistica $D^\prime = d^\prime(X_1,\, \ldots,\, X_{n})$ tale che valga: \[
                \forall \vartheta \,:\, P_\vartheta(D^\prime \leq k(\vartheta)) = 1-\alpha
            ;\] allora possiamo affermare che:
            \begin{itemize}
                \item chiamiamo intervallo di confidenza \emph{unilatero illimitato superiormente} per 
                    $k(\vartheta)$ all'$(1-\alpha)\cdot 100\%$ la quantità $(D^\prime,\, +\infty)$;
                \item se abbiamo la realizzazione $\vec{x}$ e $\bar{d}^\prime = d^\prime(\vec{x})$ allora chiamiamo 
                    l'intervallo reale $(\bar{d}^\prime,\, +\infty)$ \emph{stima intervallare unilatera};
                \item con confidenza $1-\alpha$ diremo che vale $k(\vartheta) \in (\bar{d}^\prime,\, +\infty)$.
            \end{itemize}
        \end{defn}
        \begin{defn}[Intervallo illimitato inferiormente]
            Consideriamo una statistica $D^\prime = d^\prime(X_1,\, \ldots,\, X_{n})$ tale che valga: \[
                \forall \vartheta \,:\, P_\vartheta(k(\vartheta) < D^\prime) = 1-\alpha
            ;\] allora possiamo affermare che:
            \begin{itemize}
                \item chiamiamo intervallo di confidenza \emph{unilatero illimitato inferiormente} per 
                    $k(\vartheta)$ all'$(1-\alpha)\cdot 100\%$ la quantità $(-\infty,\, D^\prime)$;
                \item se abbiamo la realizzazione $\vec{x}$ e $\bar{d}^\prime = d^\prime(\vec{x})$ allora chiamiamo 
                    l'intervallo reale $(-\infty,\, \bar{d}^\prime)$ \emph{stima intervallare unilatera};
                \item con confidenza $1-\alpha$ diremo che vale $k(\vartheta) \in (-\infty,\, \bar{d}^\prime)$.
            \end{itemize}
        \end{defn}
    ...
    \section{Esempi di intervalli notevoli}
        \subsection{Media di popolazioni normali}
            ...
            \end{note}
            \begin{prty}[\textbf{Intervallo bilatero con} $\boldsymbol{\mu =\,?,\, \sigma^2 = \sigma^2_0}$]
                ...
                .\] 
            \end{prty}
            \begin{prty}[\textbf{Intervallo unilaterale con} $\boldsymbol{\mu =\,?,\, \sigma^2 = \sigma^2_0}$]
                ...
                .\end{align*}
            \end{prty}
            \begin{prty}[\textbf{Intervallo bilatero con} $\boldsymbol{\mu =\,?,\, \sigma^2 =\,?}$]
                ...
                .\] 
            \end{prty}
            \begin{prty}[\textbf{Intervallo unilaterale con} $\boldsymbol{\mu =\,?,\, \sigma^2 =\,?}$]
        ...
        \subsection{Varianza di popolazioni normali}
            ...
            \begin{prty}[\textbf{Intervallo bilatero con} $\boldsymbol{\mu =\,?,\, \sigma^2 =\,?}$]
                ...
                \begin{align*}
                    1-\alpha &= P_{\sigma^2}\left(\chi^2_{1-\alpha /2,\,n-1} < \frac{(n-1) \cdot S_n^2}{\sigma^2} 
                    < \chi^2_{\alpha /2,\,n-1}\right) \\
                    &= P_{\sigma^2}\left(\frac{(n-1)\,S_n^2}{\chi^2_{\alpha /2,\,n-1}} > \sigma^2 
                    > \frac{(n-1)\,S_n^2}{\chi^2_{1-\alpha /2,\,n-1}}\right)
                .\end{align*}
                Deduciamo che l'intervallo di confidenza di livello $1-\alpha$ per $\sigma^2$ è definito come: \[
                    \sigma^2 \in \left(\frac{(n-1) \cdot S_n^2}{\chi^2_{\alpha /2,\,n-1}},\, 
                    \frac{(n-1) \cdot S_n^2}{\chi^2_{1-\alpha /2,\,n-1}}\right)
                .\]
            \end{prty}
            \begin{prty}[\textbf{Intervallo bilatero con} $\boldsymbol{\mu = \mu_0,\, \sigma^2 =\,?}$]
                ...
                \begin{align*}
                    1-\alpha &= P_{\sigma^2}\left(\chi^2_{1-\alpha /2,\,n} < \frac{T_n^2}{\sigma^2} 
                    < \chi^2_{\alpha /2,\,n}\right) \\
                    &= P_{\sigma^2}\left(\frac{T_n^2}{\chi^2_{1-\alpha /2,\,n}} > \sigma^2 
                    > \frac{T_n^2}{\chi^2_{\alpha /2,\,n}}\right)
                .\end{align*}
                Deduciamo che l'intervallo di confidenza di livello $1-\alpha$ per $\sigma^2$ è definito come: \[
                    \sigma^2 \in \left(\frac{T_n^2}{\chi^2_{\alpha /2,\,n}},\, 
                    \frac{T_n^2}{\chi^2_{1-\alpha /2,\,n}}\right)
                .\]
            \end{prty}
        \subsection{Media di popolazioni esponenziali}
            \begin{defn}[\textbf{Intervallo bilatero con} $\boldsymbol{\vartheta =\,?}$]
                ...
                come: \[
                    \theta \in \left(\frac{2\sum_{i=1}^{n} X_i}{\chi^2_{\alpha /2,\,2n}},\, 
                    \frac{2\sum_{i=1}^{n} X_i}{\chi^2_{1-\alpha /2,\,2n}}\right)
                .\]
            \end{defn}
        \subsection{Differenza tra medie di popolazioni normali}
            \begin{defn}[\textbf{Intervallo bilatero con} $\boldsymbol{\mu_1=\mu_2=\,?,\, \sigma^2_1 = \sigma^2_{1,\,0},\, \sigma^2_2 = \sigma^2_{2,\,0}}$]
                \hfill \\
        ...
        \subsection{Intervalli di confidenza asintotici}
            \begin{prty}
                Siano $X_1,\, \ldots,\, X_{n}$ variabili aleatorie indipendenti identicamente distribuite, 
                tali per cui $\text{E}[X_1]=\mu$ e $\text{Var}(X_1)=\sigma^2$ siano incognite; 
                sia $\bar{X}_n$ la media incognita e $S_n^2$ la varianza empirica corretta. 
                Infine valga la seguente espressione: \[
                T_n \coloneqq \frac{\sqrt{n}(\bar{X}_n - \mu)}{S_n}
                .\] Se le variabili aleatorie $X_i$ \underline{non} sono gaussiane, $T_n$ \underline{non} 
                ha distribuzione $t$\nbdash Student.
            \end{prty}
            \begin{thm}[Pivot asintotico]\label{thm:Pivot_asintotico}
                Dal Teorema~\ref{thm:Teorema_limite_centrale} e dal Teorema~\ref{thm:Legge_grandi_numeri_debole} possiamo affermare che: \[
                S_n \simeq \sigma \, \vee \, \frac{\sqrt{n}(\bar{X}_n - \mu)}{\sigma} \simeq \mathcal{N}(0,1)
                ;\] allora per $n$ abbastanza grande, ``consideriamo'' $T_n \simeq \mathcal{N}(0,1)$.
            \end{thm}
            \begin{defn}
                Usando il Teorema~\ref{thm:Pivot_asintotico} definiamo il seguente intervallo di livello $1-\alpha$  \emph{asintotico} per la media $\mu$:  \[
                \lim_{n\rightarrow +\infty} P\left\{ \bar{X}_n - z_{\alpha/2}\cdot \frac{S_n}{\sqrt{n}} \leq \mu \leq
                \bar{X}_n + z_{\alpha/2}\cdot \frac{S_n}{\sqrt{n}}\right\} = 1-\alpha
                ;\] \[
                \mu \in \left( \bar{X}_n - z_{\alpha/2}\cdot \frac{S_n}{\sqrt{n}}, 
                \bar{X}_n + z_{\alpha/2}\cdot \frac{S_n}{\sqrt{n}}\right)
                .\] Con questo intervallo possiamo stimare con un intervallo la media di una popolazione i.i.d. con media e varianza finite e incognite, in modo asintotico.
            \end{defn}
        \subsection{Intervalli di confidenza per campioni grandi}
            \begin{defn}
                ...
                ;\end{align*}
                per ogni $p$ otteniamo la confidenza sfruttando il Teorema~\ref{thm:Pivot_asintotico}: \[
                ...
                .\end{align*}
                L'ultima relazione fornisce un intervallo approssimato per $p$, che possiamo riscrivere considerando la varianza empirica $S=\sqrt{p(1-p)}$: \[
                    p \in \left(\hat{p}- z_{\alpha /2}\cdot \frac{S}{\sqrt{n}},\, 
                    \hat{p}+ z_{\alpha /2}\cdot \frac{S}{\sqrt{n}}\right) \cap [0,1]
                .\]
            \end{defn}
